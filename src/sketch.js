function sketch(p) {
  ...
  p.GridPoint = class {
    ...
    update() {
      ...
      let limit1 = 250;
      let ballAdj = fromBall < limit1 ?  this.p.map(fromBall,limit1,0,1,3) : 1;
      let ballAdj2 = fromBall2 < limit1 ?  this.p.map(fromBall2,limit1,0,1,3) : 1;
      let ballAdj3 = fromBall3 < limit1 ?  this.p.map(fromBall3,limit1,0,1,3) : 1;
      ...
      this.size = fromMouse < limit ? this.p.map(fromMouse,limit,0,p.width/750,p.width/1750) * ballAdj * ballAdj2 * ballAdj3 : p.width/750 * ballAdj * ballAdj2 * ballAdj3;
    }
